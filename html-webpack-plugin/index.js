...
HtmlWebpackPlugin.prototype.apply = function (compiler) {
  ...
  compiler.plugin('emit', function (compilation, callback) {
    ...
    Promise.resolve()
      ...
      .then(function (compilationResult) {
        var generateFunction = evaluate(compilation.assets['main.js'].source());
        var results = self.options.routes.map(function(route){
          var filename = _.endsWith(route,"/")?route + "index.html" : route;
          return generateFunction({path: route}).then(function(generated){
            return {
              generated: generated,
      ...
      }).then(function(routeInfos){
        return routeInfos.map(function(routeInfo){
          if (typeof routeInfo.compilationResult !== 'function') {
            routeInfo.html = routeInfo.compilationResult;
          } else {
            self.executeTemplate(routeInfo.compilationResult, chunks, assets, compilation,routeInfo.generated)
          }
          return routeInfo;
        })
      })
 ...
 * Returns a promise
 */
HtmlWebpackPlugin.prototype.executeTemplate = function (templateFunction, chunks, assets, compilation,generated) {
  var self = this;
